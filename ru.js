module.exports = {
  ...
  mbox: {
    notSupported: 'Похоже, этот браузер не поддерживается, или WebRTC отключён. 😢',
    ...
    roomProtected: 'Пожалуйста, введите пароль',
    ...
    badBrowser: 'У пользователя плохой браузер. Скажите ему не использовать Safari, иначе у всех могут быть баги.',
    ...
    domainNotSetUp: 'Этот домен ещё не был настроен. Если вы его владелец, пожалуйста, сделайте это из личного кабинета. А пока мы перенаправим вас на бесплатную версию.',
  ...
  upsell: {
    hd: 'HD видео',
    notFree: '<b>%s</b> доступно только с вашей личной лицензией. Советуем попробовать, первые 7 дней бесплатны, и можно отказаться.',
    startTrial: 'Попробовать',
    notInterested: 'Не интересно',
  },
  hint: {
    p2p: '<u>Данная платформа</u> использует медиа-сервера, чтобы обеспечить высокое качество во время ваших встреч.<br/><br/>• В этом режиме у нас нет доступа к вашим данным.<br/><br/>• Данные, пересылаемые через наши сервера, зашифрованы и предназначены исключительно своим получателям.<br/><br/>Однако, <u>если вам нужна максимальная приватность,</u> вы можете полностью отключить наши сервера и использовать пиринговую сеть.<br/><br/>• В этом режиме видео и аудио пересылается напрямую между пользователями, без использования серверов.<br/><br/>• Мы рекомендуем использовать этот режим, если ожидается встреча в формате один-на-один.',
  },
  modes: {
